...
export const load = async ({ locals }) => {
    ...
    const usersWithDailyInfo = await Promise.all(users.map(async (user) => {
...
export const actions = {
    updateUserRole: async ({ request, locals }) => {
        ...
        try {
            ...
            return { success: true, message: `User ${validatedData.userId} role updated to ${validatedData.role}.` };
        } catch (e) {
    ...
    updateUserCredits: async ({ request, locals }) => {
        ...
        try {
            ...
            if (validatedData.action === 'grant') {
                newCredits = await creditService.adminGrantCredits(locals.user.id, validatedData.userId, validatedData.amount);
            } else { // 'deduct'
                newCredits = await creditService.adminDeductCredits(locals.user.id, validatedData.userId, validatedData.amount);
            }

            return { success: true, message: `User ${validatedData.userId} credits ${validatedData.action}ed by ${validatedData.amount}. New balance: ${newCredits}.` };
        } catch (e) {
            if (e instanceof z.ZodError) {
                return fail(400, { message: e.errors[0].message });
            }
