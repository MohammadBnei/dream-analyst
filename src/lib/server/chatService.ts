...
class ChatService {
     ...
     */
    async saveChatMessage(dreamId: string, userId: string, role: 'user' | 'assistant', content: string, promptType?: DreamPromptType): Promise<ChatMessage> {
        if (!this.prisma) {
            this.prisma = await getPrismaClient();
        }
    ...
    async chatWithAI(
        ...
        try {
            ...
            userChatMessage = await this.saveChatMessage(dreamId, userId, 'user', userMessage, promptType);
            await creditService.deductCredits(userId, cost, 'CHAT_MESSAGE', userChatMessage.id);
        } catch (creditError) {
            console.error(`Credit deduction failed for chat message for dream ${dreamId}, user ${userId}:`, creditError);
            return new ReadableStream({
                start(controller) {
                    controller.enqueue(encoder.encode(JSON.stringify({ final: true, message: `Credit error: ${(creditError as Error).message}` }) + '\n'));
                    controller.close();
                }
            });
        }
        ...
        try {
            ...
            const messages: (SystemMessage | HumanMessage | AIMessage)[] = [
                ...
                ...history.filter(msg => msg.id !== userChatMessage.id).map(msg => {
