...
class StreamStateStore {
	...
	async updateStreamState(
		...
		if (chunk.finalStatus) {
			currentState.status =
				chunk.finalStatus === 'COMPLETED' ? StreamStatus.COMPLETED : StreamStatus.FAILED;
	...
	async isStreamOngoing(streamId: string): Promise<boolean> {
		...
		if (
			state &&
			(state.status === StreamStatus.PENDING || state.status === StreamStatus.IN_PROGRESS)
		) {
			const now = Date.now();
			...
			if ((now - state.lastUpdate) / 1000 > REDIS_STALL_THRESHOLD_SECONDS) {
				console.warn(
					`Stream ${streamId}: Detected stalled stream (last update ${state.lastUpdate}). Marking as FAILED and clearing state.`
				);
				await this.updateStreamState(
					streamId,
					{
	...
	async markStreamStarted(streamId: string, promptType: DreamPromptType): Promise<void> {
